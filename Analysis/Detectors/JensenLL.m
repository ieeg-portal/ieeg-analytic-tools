function output = JensenLL(data, fs, channels, curTime)
...
%   dbstop in ID_linelength at 23
  ...
  smoothDur = 30; % sec; default width of smoothing window
  %%---  parameters for custom feature
  ...
  timeOut = zeros(nw,1);
  featureOut = zeros(nw, length(channels));
  ...
  rmsdata = rms(data,1);
%  rmsdata(rmsdata > minThreshold) = NaN;
  ...
  for w = 1: nw
    ...
    featureOut(w,:) = IDfunction(filtOut(winBeg:winEnd,:)); 
  end
