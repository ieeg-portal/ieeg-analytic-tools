function [eventTimesUSec, eventChannels] = burst_detector_v4(dataset, channels,params)
...
parfor i = 1:numParBlocks
    ...
    for i = 1:numel(fn)
        tmp = load(fn{i});
        if(~isempty(tmp.eventTimesBlock))
            ch = cell2mat(cellfun(@(x)cell2mat(x),tmp.eventChannelsBlock,'UniformOutput',0));
           [eventTimesUSec, eventChannels] = spatialIntegration(tmp.eventTimesBlock, ch,[.25 .25]);
            eventTimesBlock_par{i} = eventTimesUSec;
            eventChannelsBlock_par{i} = eventChannels;
        end
    end
    eventTimesUSec = cell2mat(eventTimesBlock_par);
    eventChannels = vertcat(eventChannelsBlock_par{:});
end
